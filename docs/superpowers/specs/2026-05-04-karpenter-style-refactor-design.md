...
type Limits struct {
    ...
    MonthlyCostCents  *int64    `json:"monthlyCostCents,omitempty"`
}
...
type ExitClaimStatus struct {
    ...
    ExitName      string                     `json:"exitName,omitempty"`     // provider-side identifier
...
## 15. Migration story (v1alpha1 → new v1alpha1)

### Strategy

Big-bang rewrite. New CRDs live under same group `frp.operator.io` but with new kinds:

| Old | New |
|-----|-----|
| `Tunnel` (v1alpha1) | `Tunnel` (v1alpha1, new schema) |
| `ExitServer` (v1alpha1) | `ExitClaim` (v1alpha1) |
| `SchedulingPolicy` (v1alpha1) | `ExitPool` (v1alpha1) |
| (none) | `LocalDockerProviderClass`, `DigitalOceanProviderClass` (v1alpha1) |

### Migration tool

`hack/migrate-v1.go`: one-shot CLI:
1. List all `SchedulingPolicy` → emit equivalent `ExitPool` + `ProviderClass` YAML.
2. List all `ExitServer` → emit equivalent `ExitClaim` YAML.
3. List all `Tunnel` → patch with new schema.
4. Print `kubectl apply -f` instructions.
5. After verification, user deletes old CRDs.

### Conversion webhook scaffolding

Plumbed in but no-op for v1alpha1. Activated when v1beta1 ships.

### Backwards compat

**None.** This is a breaking refactor. CHANGELOG must announce; users on v1alpha1-old must migrate.
...
8. Migration tool successfully converts a sample v1alpha1-old cluster to new schema.
9. Metrics endpoint exposes the catalog in §13.
10. Feature gates plumbed; `--feature-gates` flag works.
