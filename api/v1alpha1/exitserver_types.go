...
type ExitServerSpec struct {
	...
	// foo is an example field of ExitServer. Edit exitserver_types.go to remove/update
	// +optional
...
type ExitServerStatus struct {
	...
	// conditions represent the current state of the ExitServer resource.
...
type ExitServer struct {
	...
	// spec defines the desired state of ExitServer
	// +required
	Spec ExitServerSpec `json:"spec"`

	// status defines the observed state of ExitServer
	// +optional
	Status ExitServerStatus `json:"status,omitzero"`
}
